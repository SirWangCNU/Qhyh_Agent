/**
 * Prompt B — SMART SHOT SHEET V2（导演板生图系统提示词）。
 *
 * 用途：在工坊故事板片段卡片上，把本常量与片段的 storyboard_text 拼接，
 * 直接作为 prompt 调用 POST /api/images/generate，生成该片段的导演板图。
 *
 * 注意：此 prompt 直接喂给生图模型（不经过 LLM 翻译），用户已明确接受
 * 「以实现简单换取少一次 LLM 调用」的生成质量权衡。
 */
export const STORYBOARD_BOARD_PROMPT = `Prompt B——SMART SHOT SHEET V2
